struct Node {
    ...
    void apply(ll v) { val_ += (r_+1-l_)*v; lazy_ += v;}
    void push() {
...
struct ImplicitSegTree {
    void point_update(const ll idx, const ll new_val, Node* cur_seg_node) {
        if (cur_seg_node->isLeaf()) cur_seg_node->val_ = new_val;
        else {
            ...
            if (idx <= m) point_update(idx, new_val, cur_seg_node->left_);
            else point_update(idx, new_val, cur_seg_node->right_);
            cur_seg_node->val_ = pred(cur_seg_node->left_->val_, cur_seg_node->right_->val_);
        }
    }
    ...
    void range_update(const ll l, const ll r, const ll val, Node* cur_seg_node) {
        if(l>r) {
            ...
            range_update(l,min(m,cur_seg_node->left_->r_),val,cur_seg_node->left_);
            range_update(m+1, max(m+1, cur_seg_node->right_->l_),val,cur_seg_node->right_);
            cur_seg_node->val_ = pred(cur_seg_node->left_->val_, cur_seg_node->right_->val_);
        }
    }

    ll query(const ll ql, const ll qr, Node* cur_seg_node) {
        if(cur_seg_node->l_==ql && cur_seg_node->r_==qr) {  //potentially case in which l>r is missing
            ...
            return pred(
                ...
                query(max(qm+1,cur_seg_node->right_->l_),qr,cur_seg_node->right_)
            );
        }
    }
